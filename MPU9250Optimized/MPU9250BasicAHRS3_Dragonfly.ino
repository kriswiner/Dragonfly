...
// Pin definitions
int intPin = 41;  // When mounted on back pads SDA = 42, SCL = 43, and INT = 41
bool newData = false;
uint8_t rawMPU9250Data[14];
uint8_t rawAK8963Data[8];
...
void setup()
{
  ...
  digitalWrite(myLed, LOW); // start with green led on (active LOW)
...
{
  destination[0] = ((int16_t)rawMPU9250Data[0] << 8) | rawMPU9250Data[1] ;  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = ((int16_t)rawMPU9250Data[2] << 8) | rawMPU9250Data[3] ;  
  destination[2] = ((int16_t)rawMPU9250Data[4] << 8) | rawMPU9250Data[5] ; 
  destination[3] = ((int16_t)rawMPU9250Data[6] << 8) | rawMPU9250Data[7] ;   
...
{
  if(rawAK8963Data[0] & 0x01) { // wait for magnetometer data ready bit to be set
    uint8_t c = rawAK8963Data[7]; // End data read by reading ST2 register
    if(!(c & 0x08)) { // Check if magnetic sensor overflow set, if not then report data
      destination[0] = ((int16_t)rawAK8963Data[2] << 8) | rawAK8963Data[1] ;  // Turn the MSB and LSB into a signed 16-bit value
      destination[1] = ((int16_t)rawAK8963Data[4] << 8) | rawAK8963Data[3] ;  // Data stored as little Endian
      destination[2] = ((int16_t)rawAK8963Data[6] << 8) | rawAK8963Data[5] ; 
   }
  }
}
