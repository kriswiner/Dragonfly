/* EM7180_MPU9250_MS5637_Dragonfly Basic Example Code
 by: Kris Winer
 date: August 31, 2016
 ...
 Sketch runs on the 3.3 V 80 MHz Dragonfly (STM32L476)
 ...
 host microcontroller. If the read rate is infrequent enough (2 Hz is sufficient since pressure and temperature
 do not change very fast), then the sensor fusion rate is not significantly affected.
 
 This sketch uses SDA/SCL on pins 20/21 (I2C0), respectively, and it uses the Dragonfly Wire.h library.
 ...
 4k7 resistors are on the EM7180+MPU9250+MS5637+M24512DFC Ultimate Sensor Fusion Solution
 
 Hardware setup:
 EM7180 USFS ------------- Dragonfly
 VDD ---------------------- 3.3V
 SDA ----------------------- 20
 SCL ----------------------- 21
 GND ---------------------- GND
...
// Pin definitions
int intPin = 9;  // This can be changed to any available GPIO
bool newData = false;
...
void setup()
{
  Wire.begin(0, 400000, false); // set master mode on pins 20/21, I2C frequncy at 400 kHz
  Serial.begin(38400);
  delay(4000);
 ...
 // Set up the SENtral as sensor hub in normal operating mode
  if(!passThru) {
 ...
 //
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    Serial.print(lin_Az*1000, 2);  Serial.println(" mg");
      ...
      Serial.print("Altimeter temperature = "); 
      if(passThru) Serial.print(9.0f*((float)Temperature)/5.0f + 32.0f, 2); 
      if(!passThru) Serial.print(9.0f*temperature/5.0f + 32.0f, 2); 
